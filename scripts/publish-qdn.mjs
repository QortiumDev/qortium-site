...
// Publishes the built static site (dist/) to Qortium QDN as a WEBSITE resource,
// using the local preview account that the other Qortium apps publish with.
// Adapted from qortium-trust/scripts/publish-qdn.mjs. Overridable via env vars
// with the QORTIUM_SITE_ prefix.
const DEFAULT_NODE_API_URL = 'http://127.0.0.1:24891';
const DEFAULT_NAME = 'Qortium';
// Publish under the non-default "Qortium" identifier -> qdn://WEBSITE/Qortium/Qortium.
// Multi-page navigation under a non-default identifier works since Core renders the
// identifier as a path segment (qortium-core #43) and Home builds path-segment render
// URLs (qortium-home #46); the name's default-website slot stays free.
...
const nodeApiUrl = (process.env.QORTIUM_SITE_NODE_API_URL ?? DEFAULT_NODE_API_URL).replace(/\/+$/, '');
const publishName = process.env.QORTIUM_SITE_QDN_NAME ?? DEFAULT_NAME;
const identifier = process.env.QORTIUM_SITE_QDN_IDENTIFIER ?? DEFAULT_IDENTIFIER;
const publishTitle = process.env.QORTIUM_SITE_QDN_TITLE ?? DEFAULT_TITLE;
const service = process.env.QORTIUM_SITE_QDN_SERVICE ?? DEFAULT_SERVICE;
...
function getConfiguredApiKeyPath(settings, cwd) {
  const configuredPath = settings && typeof settings.apiKeyPath === 'string' ? settings.apiKeyPath.trim() : '';
  const apiKeyDirectory = configuredPath
...
function getApiKeySource() {
  const explicitApiKey = process.env.QORTIUM_SITE_NODE_API_KEY?.trim();

  if (explicitApiKey) {
    return { apiKey: explicitApiKey };
  }

  if (process.env.QORTIUM_SITE_NODE_API_KEY_PATH?.trim()) {
    return { apiKey: readText(apiKeyPath) };
  }

  const runningCoreApiKeyPath = getRunningLocalCoreApiKeyPath();

  if (runningCoreApiKeyPath) {
    return { apiKey: readText(runningCoreApiKeyPath) };
  }
...
async function getNameInfo(name) {
  ...
  // Core returns an error object (not 404) for an unregistered name.
  if (info && (info.error || !info.owner)) {
    return null;
  }
